...
class EventService extends Service {
	...
	//
	async create(params) {
		const res = this.ctx.model.EventsManage.create(params);
	...
	async update(uuid, params) {
		const res = this.ctx.model.EventsManage.update(
			params
			, {
				where: {
					uuid,
	...
	// 获取单条数据信息
	async getInfo(uuid) {
		const res = await this.ctx.model.EventsManage.findOne({
			where: {
				uuid,
			},
			raw: true,
  ...
  // 获取多条数据信息
    ...
    const condition = {
      retirement_status: '1',
		...
		const res = await this.ctx.model.EventsManage.findAndCountAll({
			where: condition,
			raw: true,
		})
		return res;
	...
	// max min
	async getDateMaxMin(schedule_uuid, resource_uuid = '') {
    const condition = {
      retirement_status: { [this.ctx.model.Sequelize.Op.ne]: '3' },
      from: '',
    }
    if(schedule_uuid) {
      condition.schedule_uuid = schedule_uuid;
    }
    if(resource_uuid) {
      condition.shots_uuid = resource_uuid;
    }
		const res = await this.ctx.model.EventsManage.findOne({
			attributes: [[this.ctx.model.EventsManage.sequelize.fn('MAX', this.ctx.model.EventsManage.sequelize.col('end_at')), 'end'], [this.ctx.model.EventsManage.sequelize.fn('MIN', this.ctx.model.EventsManage.sequelize.col('start_at')), 'start']],
			raw: true,
			where: condition,
		})
		return res;
